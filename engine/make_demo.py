...
for scene,duration in zip(scenes,durations):
    words=scene['text'].split(); chunks=[]; current=[]
    for word in words:
        if len(' '.join(current+[word]))>88: chunks.append(current);current=[]
        current.append(word)
    if current:chunks.append(current)
    t=elapsed
    for chunk in chunks:
        end=t+(duration-.6)*len(chunk)/len(words)
        split=len(chunk)//2
        text=' '.join(chunk) if len(' '.join(chunk))<=48 else ' '.join(chunk[:split])+'\n'+' '.join(chunk[split:])
        subtitles.append(f'{idx}\n{stamp(t)} --> {stamp(end)}\n{text}\n');idx+=1;t=end
    elapsed+=duration
(out/'demo.srt').write_text('\n'.join(subtitles),encoding='utf-8')
(out/'transcript.md').write_text('# Mosaic Mix Lab demo transcript\n\nSynthetic voice: Microsoft Zira Desktop. Authentic screenshots of the completed app.\n\n'+'\n\n'.join(s['text'] for s in scenes)+'\n',encoding='utf-8')
...
validation={'durationSeconds':round(total,3),'resolution':'1920x1080','fps':24,'videoCodec':'H.264','audioCodec':'AAC 128kbps','pixelFormat':'yuv420p','fastStart':True,'captionCues':idx-1,'captions':'burned in and separate SRT','voice':'Microsoft Zira Desktop, synthetic','scenes':len(scenes),'source':'Authentic app screenshots captured through browser tool','fileBytes':(out/'mosaic-mix-lab-demo.mp4').stat().st_size,'audioLevels':re.findall(r'(?:mean|max)_volume: [^\n]+',probe.stderr)}
